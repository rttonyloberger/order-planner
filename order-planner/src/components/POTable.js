    ...
    <div>
      ...
      <div style={{ overflowX: 'auto', border: '1px solid #ddd', borderRadius: 8, marginBottom: 4 }}>
        <table style={{ borderCollapse: 'collapse', width: '100%' }}>
          <thead>
            <tr>
              <th style={thS}>Supplier</th><th style={thS}>PO #</th><th style={thS}>Status</th>
              {isSG && <th style={thS}>Product</th>}
              <th style={thS}>Dest</th><th style={thS}>Order Date</th><th style={thS}>ETA</th>
              <th style={thS}>PO Value</th><th style={thS}>Tracking</th>
                ...
                <tr key={p.id} style={{ background: sc.bg + '18', borderBottom: '1px solid #f0f0f0' }}>
                  ...
                  <td style={tdS}>
                    <select style={{ ...selS, background: sc2.bg, color: sc2.fc, borderColor: sc2.fc + '44' }} value={p.status || 'Draft'} onChange={e => handleStatus(p, e.target.value)}>
                      <option value='Draft'>Draft</option><option value='Committed'>Committed</option>
                      <option value='Complete'>Complete</option><option value='Delete'>Delete</option>
                    </select>
                  </td>
                  {isSG && (
                    <td style={tdS}>
                  ...
                  <td style={tdS}><input type="date" defaultValue={p.eta || ''} onBlur={e => update(p, 'eta', e.target.value)} style={dateInputS} /></td>
                  ...
                  <td style={{ ...tdS, minWidth: 130 }}>
                    {p.tracking_url
                      ? <><a href={p.tracking_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 10, color: '#0C447C' }}>Track shipment</a><br /><span style={{ fontSize: 9, color: '#C00000', cursor: 'pointer', textDecoration: 'underline' }} onClick={() => update(p, 'tracking_url', null)}>remove</span></>
                      : <input type="text" placeholder="Paste URL" style={{ fontSize: 10, padding: '3px 5px', border: '1px solid #ddd', borderRadius: 4, width: '100%' }} onBlur={e => e.target.value && update(p, 'tracking_url', e.target.value)} />
                    }
                  </td>
                  {showShip && (
