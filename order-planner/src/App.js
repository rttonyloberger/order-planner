...
const TAB_COLORS = {
  Control:   { base: '#375623', active: '#1E6B3C', hover: '#1E6B3C' },
  Receiving: { base: '#7B3F00', active: '#5C2E00', hover: '#5C2E00' },
  RT:        { base: '#4472C4', active: '#1F3864', hover: '#2E4D7B' },
  SG:        { base: '#4472C4', active: '#1F3864', hover: '#2E4D7B' },
}

export default function App() {
  const store = useStore()
  const [activeTab, setActiveTab] = useState('Control')
  const [modal, setModal] = useState(null) // { title, body, onConfirm, confirmLabel, danger }
  ...
  return (
    <div style={{ padding: '24px 36px 60px', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', fontSize: 13, color: '#1a1a1a', background: '#f8f8f6', minHeight: '100vh' }}>
      {/* Top bar */}
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 0 }}>
        <div style={{ display: 'flex', borderBottom: '2px solid #1F3864', flex: 1 }}>
          {TABS.map(tab => {
      ...
      {activeTab === 'Control' && (
        <ControlTab
          rtConfig={store.rtConfig} sgConfig={store.sgConfig}
      ...
      {activeTab === 'RT' && (
        <RTTab
          pos={store.pos} calState={store.calState} rtConfig={store.rtConfig}
          months={months} upsertPO={store.upsertPO} deletePO={store.deletePO}
          upsertCalState={store.upsertCalState}
          showModal={showModal} closeModal={closeModal}
        />
      )}
      {activeTab === 'SG' && (
        <SGTab
          pos={store.pos} calState={store.calState} sgConfig={store.sgConfig}
          months={months} upsertPO={store.upsertPO} deletePO={store.deletePO}
          upsertCalState={store.upsertCalState}
          showModal={showModal} closeModal={closeModal}
        />
...
const rollBtnStyle = {
  ...
  borderRadius: 6, fontSize: 12, fontWeight: 500, cursor: 'pointer', marginLeft: 8, marginBottom: 2
}
